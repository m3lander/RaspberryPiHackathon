...
def extract_search_terms_from_description(client, description):
    """
    ...
    response = client.models.generate_content(
        model="gemini-3-flash-preview",
        contents=["Given the very detailed description, output a more compact description suitable for search purposes in a search engine. Your response must be a single sentence.", description]
    )
